...
if st.session_state.current_q < len(quiz_data):
    ...
    if st.button("こたえを決定！ / Submit Answer!"):
        ...
        if correct:
            ...
            st.audio("correct.mp3", format="audio/mp3")
            st.session_state.score += 1
        else:
            st.error("❌ ざんねん… / Incorrect…")
            st.audio("wrong.mp3", format="audio/mp3")

        st.info(f"せいかいは：{q['options'][q['answer']]} / Correct answer: {q['options'][q['answer']]}\n\n{q['explanation']}")
        st.info(f"せいかいは：{q['options'][q['answer']]} / Correct answer: {q['options'][q['answer']]}\n\n{q['explanation']}")
        st.rerun()
...
# ----- 結果表示 / Results -----
    ...
    st.audio("finish.mp3", format="audio/mp3")
